...
2.3. Ask Jouni Helske and the other `KFAS` and `bssm` contributors about supporting dimnames and an optional `Time` and `dT` component(s)) of an `SSModel` plus a function `asTime` = `Time` component of a model else `as.Posixct(names(y))` else `as.Date(names(y))` else `as.numeric(names(y))` else [`ordered(names(y))` with `dT = `1:length(y)`] with `dT_ = diff(Time)` and `dT = c(dT[1], dT)` and for `bssm` assuming Student's t migrations of `growthRate`.  
