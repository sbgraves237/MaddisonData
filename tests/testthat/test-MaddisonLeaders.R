test_that("MaddisonLeaders", {
  ...
  expect_equal(colnames(Leaders0), c('yearBegin', 'yearEnd', 'ISO'))
  L0NA <- sapply(Leaders0, function(x)sum(is.na(x)))
  expect_equal(L0NA, c(yearBegin=0, yearEnd=0, ISO=0))
...
# economies 
  ...
  expect_equal(colnames(Leaders1), c('yearBegin', 'yearEnd', 'ISO'))
  L1NA <- sapply(Leaders1, function(x)sum(is.na(x)))
  expect_equal(L1NA, c(yearBegin=0, yearEnd=0, ISO=0))
