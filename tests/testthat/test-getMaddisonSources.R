test_that("getMaddisonSources", {
  ...
  MDPonly12 <- getMaddisonSources(names(MaddisonSources)[1:12], FALSE)#only MDP 
  ...
  MDP1 <- tibble::tibble(ISO=character(2), 
    ...
    source=c(paste('GDP pc:    Total Economy Database (TED) of the',
              ...
              paste('population:Total Economy Database (TED) of the', 
                ...
                'Otherwise UN national accounts statistics') ) )
  MDPall_ <- MDP1 
  for(iso in names(MaddisonSources)){
    isoi <- cbind(ISO=iso, MaddisonSources[[iso]])
...
# GBR
  GBR1 <- cbind(ISO='GBR', MaddisonSources[['GBR']])
...
# GBR_USAsince1500
  GBRsince1500 <- GBR1[-1,]
  USA_ <- cbind(ISO='USA', MaddisonSources[['USA']])
