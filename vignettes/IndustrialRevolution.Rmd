...
MaddisonLeaders <- function(except=character(0), y='gdppc', 
                            group='ISO', data=MaddisonData){
  years <- table(MaddisonData$year)
  nYrs <- length(years)
  ...
  for(i in 1:nCtries){
    dati <- subset(MaddisonData, ISO==ctries[i])
    yrsi <- as.character(dati$year)

    seleq <- which(Leaders[yrsi, 'maxGDPpc']==dati$gdppc)
    if(length(seleq)>0){
      Leaders[yrsi, 3][seleq] <- 
        paste0(Leaders[yrsi, 3][seleq], ":", dati[seleq, 'ISO'])
    }
...
Leaders0 <- MaddisonLeaders()
Leaders00 <- table(Leaders0$ISO)
rownames(MaddisonCountries) <- MaddisonCountries$ISO
MaddisonCountries[names(Leaders00), 1:2]
```

Let's redo this without countries like `ARE`, `KWT`, and `QAT` that have clearly NOT been technology leaders. 

```{r leaders1}
Leaders1 <- MaddisonLeaders(c('ARE', 'KWT', 'QAT'))
Leaders10 <- table(Leaders1$ISO)
MaddisonCountries[names(Leaders10), 1:2]
```
