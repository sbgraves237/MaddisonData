...
BlackDeathYrs <- c(1346, 1354)
abline(v=BlackDeathYrs, col='grey', lty='dotted')

BlackDeath <- data.frame(x=mean(BlackDeathYrs), y=3, 
                         label='Black Death', srt=90)
GBR1 <- ggplotPath('year', 'gdppc', data=selMadd, scaley=1000,
      ylab='GBR GDPpc (2011 K$ppp)', vlines=BlackDeathYrs,
      labels=BlackDeath)
