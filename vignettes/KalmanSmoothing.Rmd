...
These data suggest changes in 1291 and 1296. [The Welsh rebel leader Rhys ap Maredudd](https://en.wikipedia.org/wiki/Rhys_ap_Maredudd) was captured in 1291, and Scots defeated England in the [Battle of Stirling Bridge](https://en.wikipedia.org/wiki/Battle_of_Stirling_Bridge) in 1297. 

The [Black Death](https://en.wikipedia.org/wiki/Black_Death) devastated Europe from 1346 to 1353 and [England](https://en.wikipedia.org/wiki/Black_Death_in_England) in 1348 and 1349, returning again in 1361 and 1362. It ended the [Hundred Years' War](https://en.wikipedia.org/wiki/Hundred_Years%27_War) between England and France in 1353, though another major campaign in France was launched in 1355.  
...
This suggests turning points in the slope in 1375 and 1392. The Wikipedia ["Timeline of English history"](https://en.wikipedia.org/wiki/Timeline_of_English_history#13th_century) does not suggest any obvious events associated during that period that might have driven the apparent increase in `GDPpc` during that period. We will not mark this on the plot. 

The next major turning point was in 1649 when the English decapitated [King Charles I](https://en.wikipedia.org/wiki/Charles_I_of_England) during the [English Civil War](https://en.wikipedia.org/wiki/English_Civil_War) (1642 to 1651). There was economic instability during the reign of [Queen Ann](https://en.wikipedia.org/wiki/Anne,_Queen_of_Great_Britain) (1702 to 1714). And the slope increased after the [Napoleonic Wars](https://en.wikipedia.org/wiki/Napoleonic_Wars)
...
The plot indicates that the rate of economic growth of the United Kingeom began to increase after 1819. That's the year of the [Peterloo Massacre](https://en.wikipedia.org/wiki/Peterloo_Massacre#Reaction_and_aftermath) during which a crowd of roughly 60,000 humans assembled nonviolently to  demand a reform of parliamentary representation and were attacked by a regiment of the English cavalry. Eighteen were killed and between 500 and 700 were injured. While the authorities initially cracked down on dissent and freedom of the press, Peterloo marked "the point of final conversion of provincial England to the struggle for enfranchisement of the working class", as described by `r Citet(bib, "White57")`. 
...
GBRlbls <- data.frame(x=c(mean(turningPts[1:2]),
                       ...
                       y=c(5, 3, 5, 5, 6, 4, 6), 
          label=c('Conflicts with Welsh and Scots', 'Black Death', 
...
```{r GBRgrowthFit}
growthUpdateFn1 <- function(pars, model){
# 1 parameter = variances of both 1-year migration and observation.
  nobs <- nrow(selMadd)
...
GBRgrowthMdl2 <- growthUpdateFn1(EWMA1fit1$optim.out$par, selMadd2m)
class(GBRgrowthMdl2)
is.SSModel(GBRgrowthMdl2)

GBRgrowthFit1 <- fitSSM(selMadd2m, inits=EWMA1fit1$optim.out$par, 
                        method = "BFGS", updatefn = growthUpdateFn1)
(GBRgrowthLogLik1 <- logLik(GBRgrowthFit1$model))
EWMA1logLik1
```

Problem: `GBRgrowthLogLik1` = `r GBRgrowthLogLik1` is much than `EWMA1logLik1` = `r EWMA1logLik1`: It should be greater, unless I misunderstand the likelihood computation. 

If we only allow `Q[1, 1]` to be nonzero, then `fitSSM(selMadd2m, ...)` should be equivalent to `EWMA1fit1`.

```{r GBRgrowthFit0}
growthUpdateFn0 <- function(pars, model){
# 1 parameter = variances of both 1-year migration and observation.
  nobs <- nrow(selMadd)
  v1 <- as.numeric(exp(pars[1]))
  model$H[1,1, 1] <- v1 # observation variance 
#
  Q2 <- array(0, c(2, 2, nobs) )
# chol(Q2[,,i]) = (sqrt(dYr)) sqrt(choose(dYr, 2))*sqrt(sigma)
#                 (    0           sqr(dYr)     )
# so 
# Q2[,,i] = (         dYr          sqrt(dYr*choose(dYr, 2)))*sigma
#         = (sqrt(dYr*choose(dYr, 2))       dYr           )  
...
is.SSModel(selMadd2m)
GBRgrowthMdl0 <- growthUpdateFn0(EWMA1fit1$optim.out$par, selMadd2m)
class(GBRgrowthMdl0)
is.SSModel(GBRgrowthMdl0)

GBRgrowthFit0 <- fitSSM(selMadd2m, inits=EWMA1fit1$optim.out$par, method = "BFGS", 
                   updatefn = growthUpdateFn0)
GBRgrowthLogLik1
EWMA1logLik1
(GBRgrowthLogLik0 <- logLik(GBRgrowthFit0$model))
```

Interesting: `GBRgrowthLogLik0 > GBRgrowthLogLik1`, which suggests a problem somewhere that I do not understand. 

Let's ignore this for the moment and plot the level and slope from `GBRgrowthLogLik1`. 

```{r GBRgrowthLogLik1plot}
(EWMA2kfs <- KFS(GBRgrowthFit1$model))
(EWMA2kfs0 <- KFS(GBRgrowthFit0$model))
...
plot(exp(a_sm[, 1])/1000, las=1, log='y', xlim=c(1910, 1950), 
     ylim=c(6,13))
abline(v=turningPts, lty='dotted', col='grey')
...
EWMA2pred <- predict(GBRgrowthFit1$model, interval='prediction')
head(EWMA2pred, 3)
tail(EWMA2pred, 2)
