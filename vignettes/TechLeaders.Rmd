...
For simpicity, we focus on `NLD`, `GBR`, `USA`, and `SGP`. A few other countries (`AUS`, `NZL`, `CHE`, and `LUX`) led for so few years in this period that including them in this plot would likely add more complexity than informtion and make it harder to understand the big picture. 
...
We want to annotate this plot in multiple ways: 
- Annotate the `NLD` line as "Holland" (at `ggppc` = roughly $5K in 1600) and "Netherlands" (at `gdppc` = roughly $4K in 1900).
...
- `WW1` (1914-1918). 
- The presidency of [Herbert Hoover](https://en.wikipedia.org/wiki/Herbert_Hoover) (1929-1933).
- The presidency of [Franklin Roosevelt](https://en.wikipedia.org/wiki/Franklin_D._Roosevelt) (1933-1945). 
- `WW2` (1939-1945).

```{r NLD_SGP1}
x0 <- yr(c('1642-01-04', '1812-06-18', '1861-04-12', '1914-07-28', 
           '1929-03-04', '1933-03-04', '1939-09-01')) 
x1 <- yr(c('1651-09-03', '1815-02-17', '1865-05-26', '1918-11-11', 
           '1933-03-04', '1945-04-12', '1945-09-02'))
Vlines <- sort(unique(c(x0, x1)))
attr(Vlines, 'color') <- c(rep('grey', 8), 'red', 'green4', 'grey', 
                           'green4', 'grey')
Hlines = c(1, 3, 5, 10, 30, 50)
Lbls <- data.frame(x=c(1500, 1600, 1740, 1870, (x0+x1)/2, 1985), 
                   y=c(1.35,  5.7, 1.65,  3.5, 13, rep(15, 3), 36, 21, 64, 8), 
    label=c('UK', 'Holland', 'US', 'Netherlands', 'English civil war', 
            'War of 1812', 'American Civil War', 'WW1', 'Hoover', 'FDR', 
            'WW2', 'Singapore'), 
    srt=c(0, 0, 40, 50, rep(90, 7), 87), 
    col=c('red', 'orange', 'blue', 'orange', 'red', rep('grey', 3), 'red',
